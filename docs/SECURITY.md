...
| Unknowing plain-HTTP remote use | Non-loopback HTTP requires the explicit persisted `allow_insecure_http` acknowledgement; test/configure refuse with `insecure_http` otherwise; UI renders acknowledged state as INSECURE |
